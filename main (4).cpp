...
vector<pair<int, string>> voting()
{
    vector<pair<int, string>> votes(idStakes.size());
...
{
    stringstream ss;
    ss << PatientName << ProfessionalName << sex << uniqueID << diagnosis << fee << tTime;

    return sha256(ss.str());
}

// Function to print all the transaction history related to a particular land
void TransactionHistory(string ID, vector<Transaction> &allTrans)
{
    int cnt = 0;
    for (int i = 0; i < allTrans.size(); i++)
    {
        if (allTrans[i].uniqueID == ID)
        {
            cnt++;
            cout << "The patient with name " << allTrans[i].PatientName << " has been diagnosed by " << allTrans[i].ProfessionalName << " with the diagnosis " << allTrans[i].diagnosis << ".\n The patient is a " << allTrans[i].sex << " and the fee paid by him is " << allTrans[i].fee << " dated " << allTrans[i].tTime << "\n";
        }
    }
    if (cnt == 0 && allTrans.size() != 0)
    {
        cout << "This Patient has not been diagosed yet!\n";
    }
    return;
}
...
void assignStakes(vector<Transaction> &allTrans, string idx)
{
...
{
    stringstream ss;
    ss << nIndex << sPrevHash << trans1.tTime << trans1.PatientName << trans1.ProfessionalName << trans1.sex << trans1.diagnosis << trans1.fee << trans2.tTime << trans2.PatientName << trans2.ProfessionalName << trans2.sex << trans2.diagnosis << trans2.fee << trans3.tTime << trans3.PatientName << trans3.ProfessionalName << nNonce;
...
{
    vector<pair<int, string>> votes = voting();

    string str1 = votes[0].second;
    string str2 = votes[1].second;
    string str3 = votes[2].second;
    int n1 = str1.length();
    int n2 = str2.length();
    int n3 = str3.length();
    ...
    do
    {
        nNonce++;
        sHash = CalculateHash();
        if (sHash.substr(0, n1) == str1.substr(0, n1))
            miner = str1;
        if (sHash.substr(0, n2) == str1.substr(0, n1))
            miner = str2;
        if (sHash.substr(0, n3) == str1.substr(0, n1))
            miner = str3;
    } while (miner == "-1");
...
{
    Transaction dummy;
    cout << "Please enter the unique Id\n";
    cin >> dummy.uniqueID;
    cout << "Please enter the Patient Name\n";
    cin >> dummy.PatientName;
    cout << "Please enter the Professional Name\n";
    cin >> dummy.ProfessionalName;
    cout << "Please enter the sex\n";
    cin >> dummy.sex;
    cout << "Please enter the diagnosis\n";
    cin >> dummy.diagnosis;
    cout << "Please enter the fee paid\n";
    cin >> dummy.fee;
    allTrans.push_back(dummy);
    transHash.push_back(dummy.transhash);
    assignStakes(allTrans, dummy.PatientName);
    assignStakes(allTrans, dummy.ProfessionalName);
    cout << "\nYour data has been securely saved with us !\n";
}

int main()
{
    transHash.push_back(sha256("initial"));
    transHash.push_back(sha256("data"));
    transHash.push_back(sha256("given"));
    transHash.push_back(sha256("database"));
    ...
    while (userIP != 3) // user does not want to exit
    {
        ...
        else if (userIP == 2) // if the user wants to review the transactions carried out in the past on a particular land
        {
            // cout << "cat\n";
            string id = "";
            cout << "\nEnter the Unique ID whose details are to be viewed: \n";
            cin >> id;
    ...
    if (userIP == 3 && transCount != 0 && transCount % 4 != 0)
    {

        cout << "\nNeed atleast 4 transactions to mine a complete block\n"
             << "Current Transactions: " << transCount % 4 << "\n";
        if (transCount % 4 == 1)
            bChain.AddBlock(Block(transCount / 4 + 1, *(lastPointer - 1), *(lastPointer - 1), *(lastPointer - 1), *(lastPointer - 1)));
        // else if (transCount % 4 == 2)
        //     bChain.AddBlock(Block(transCount / 4 + 1, *(lastPointer - 1), *(lastPointer - 1), *(lastPointer - 2), *(lastPointer - 2)));
        // else if (transCount % 4 == 3)
        //     bChain.AddBlock(Block(transCount / 4 + 1, *(lastPointer - 1), *(lastPointer - 2), *(lastPointer - 3), *(lastPointer - 3)));
    }
